...
//   [1] cb_id             - relay circular buffer index
//   [2] pages_per_entry   - CB pages one delivered entry covers
//   [3] num_entries       - entries to consume
...
void kernel_main() {
    ...
    constexpr uint32_t pages_per_entry = get_compile_time_arg_val(2);
    constexpr uint32_t num_entries = get_compile_time_arg_val(3);
    ...
    for (uint32_t entry = 0; entry < num_entries; ++entry) {
        relay_cb.reserve_back(pages_per_entry);
        pipe.wait_front(entry == 0 ? 1u : 2u);
        relay_cb.push_back(pages_per_entry);
        if (entry >= 1) {
