...
void detail::ProgramImpl::set_prefetcher_pipe_relay_slot(
    ...
    for (const CoreCoord& core : corerange_to_cores(receiver_cores)) {
        ...
        TT_FATAL(
            ...
            prefetcher_pipe_id,
            core.str());
        // One delivered entry has to publish a whole number of relay pages, so the relay can be
        // finer-grained than the pipe entry (a tile-paged CB under a K-block entry) but never coarser
        // and never a ragged fraction.
        TT_FATAL(
            participant->entry_size % relay_entry_size == 0,
            "PrefetcherPipe relay entry size {} must divide the Attach dense entry_size {} on core {}",
            relay_entry_size,
