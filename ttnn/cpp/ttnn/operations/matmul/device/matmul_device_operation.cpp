...
void validate_prefetcher_pipes_mcast_in0_geometry(
    ...
    // A pipe never resizes: the entry it was created with has to be exactly the in1 K-block the
    // reader publishes, or the delivered pages and the CB pages disagree by a fixed stride.
    const uint32_t in1_block_size_bytes =
        program_config.in0_block_w * program_config.per_core_N *
        in1_tile.get_tile_size(tt::tt_metal::datatype_to_dataformat_converter(input_tensor_b.dtype()));
    TT_FATAL(
        prefetcher_pipes.entry_size == in1_block_size_bytes,
        "mcast_in0 prefetcher_pipes entry_size {} B must equal this matmul's in1 K-block size {} B "
        "(in0_block_w {} * per_core_N {} * {} B per tile); PrefetcherPipes do not resize, so create them with that "
        "entry_size",
        prefetcher_pipes.entry_size,
        in1_block_size_bytes,
        program_config.in0_block_w,
        program_config.per_core_N,
        in1_tile.get_tile_size(tt::tt_metal::datatype_to_dataformat_converter(input_tensor_b.dtype())));

    // The reader streams K-blocks through a two-entry window: publish the current block, then wait
    // for compute to drain the previous one before returning its credit.
    TT_FATAL(
        prefetcher_pipes.num_entries >= 2,
        "mcast_in0 prefetcher_pipes requires a two-entry streaming window, but the pipes hold {} entries",
        prefetcher_pipes.num_entries);
}
